...
file=(start.sh setupServer.sh startup.sh context.sh nginxConfig.sh setupDav.sh letsencrypt.sh nginxStart.sh sslConfig.sh functions.sh configDav.sh test.sh)
...
for name in ${nameServers[*]} ; do
    [[ ! $(sed -e "/^nameserver $name/ !d" $file) ]] && echo 'nameserver '$name >> $file
    done
cat $file
echo $LINE$LINE
return
